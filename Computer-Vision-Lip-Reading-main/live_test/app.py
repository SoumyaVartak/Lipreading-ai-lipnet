...
# # Define the input shape (same as your code)
# input_shape = (17, 80, 112, 3)

# # Define the model architecture (same as your code)
...
# # Load the weights (assuming the file is in the same directory)
# model.load_weights("C:/Users/saumy/OneDrive/Desktop/mainpro/Computer-Vision-Lip-Reading-main/live_test/model_weights.h5")

# # Load the detector
# detector = dlib.get_frontal_face_detector()

# # Load the predictor
# predictor = dlib.shape_predictor("C:/Users/saumy/OneDrive/Desktop/mainpro/Computer-Vision-Lip-Reading-main/data/face_weights.dat")

# # Initialize variables (same as your code)
...
import sys
sys.path.append('/path/to/parent/directory')  # Replace with the actual path to the parent directory
from joblib import load
#from utils.conversion import num_to_char

import tensorflow as tf

# Define the function to convert numbers to characters
@tf.function
def num_to_char(num):
    return tf.strings.unicode_encode([num], 'UTF-8')[0]
...
def decode_predictions(predictions):
    # Greedy decoding
    decoded_text = ''.join([num_to_char(prediction) for prediction in predictions])
    return decoded_text
...
def gen_frames():
    ...
    while True:
        ...
        if not success:
            ...
            # Process frame
            processed_text = process_frame(frame)

            # Draw processed text on the frame
            cv2.putText(frame, processed_text, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
# Load LipNet model
model = load("C:/mainpro/Computer-Vision-Lip-Reading-main/live_test/model_weights.h5")
